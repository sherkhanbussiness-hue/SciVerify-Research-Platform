...
function OverviewContent() {
  ...
  const { summary, evaluations, tasks, models, error } = useHarnessView();
    ...
    <div className="space-y-6">
      {error && (
        <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-xs text-red-300">
          Error loading evaluation telemetry: {error instanceof Error ? error.message : String(error)}
        </div>
      )}
...
function ComparisonPage() {
  const { summary, models, results, error } = useHarnessView();
